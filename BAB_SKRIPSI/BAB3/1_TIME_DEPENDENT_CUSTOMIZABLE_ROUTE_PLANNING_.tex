...
\Procedure{TAMLBD}{$as, at, G, OG$}
    ...
    \While{$|fQ|+|foQ| > 0 \textbf{ dan } |bQ|+|boQ| > 0$ }
        ...
        \If {$minQ < minOQ$}
            \State \Call{BDTurnAwareDijkstra}{$s,t,fInfo,bInfo,fQ, bQ, foQ, boQ, G, OG, shortest, Via$}
        \Else 
             \State \Call{BDTurnAwareOverlayDijkstra}{$s,t,fInfo,bInfo,fQ, bQ, foQ, boQ, G, OG, shortest, Via$}
        \EndIf
    \EndWhile   
    \State \textbf{return} $Via, shortest, $ \Call{PATHUNPACKING}{$fInfo, bInfo$}, $fInfo, bInfo$ 
\EndProcedure
\end{algorithmic}
\end{minipage}%
}
...
\begin{algorithmic}[1]
\setstretch{0.9}
\Procedure{BDTurnAwareDijkstra}{$s,t,fInfo,bInfo,fQ, bQ, foQ, boQ, G, OG, shortest, Via$}
       ...
       \State  $((u, uEntryPos), \cdot) \gets fQ.deleteMin()$ \Comment{\textit{forward search} pada graf $G$}
        \For{$(oArc, turn, v) \in G.E[u,uEntryPos]$}
            ...
            \If{$l_{st}(v)=0$}
                ...
                \State \Call{RELAX}{$v,u,uEntryPos, oArc, false, newTT,fInfo,bInfo, fQ,bQ, G, shortest, Via$}
            \Else 
                \State \Call{RELAXOVERLAY}{$v,u,uEntryExitPos,oArc,false, shortest, G, newTT, l_{st}(v), fInfo,\newline bInfo, foQ, boQ, Via$}
            \EndIf 
        \EndFor 
    \Else 
        ...
        \For{$(iArc, turn, v) \in  G.\overleftarrow{E}[u,uExitPos]$}
            ...
            \If{$l_{st}(v)=0$}
                ...
                \State \Call{RELAX}{$v,u,uExitPos,iArc,true,newTT,fInfo,bInfo,fQ,bQ, G, shortest, Via$}
            \Else
...
\begin{algorithmic}[1]
\setstretch{0.9}
\Procedure{RELAX}{$v,u,uEntryPos, Arc, backward, newTT, fInfo,bInfo, fQ,bQ, G, shortest, Via$}
    \If {$backward = false$}
        ...
        \If {$newTT < fInfo[vEntryPos].tt \textbf{ or } vVisited = false$}
            ...
            \For{$(voArc, vExitPos, turn, \cdot) \in G.E[v, arc.entryPos]$} 
                ...
                \If{$visitedByBackward = true$}
                    \State $newTT \gets fInfo[vEntryPos].tt +  binfo[vExitPos].tt$
                    \If{$newTT < shortest$}
                        \State $shortest=  newTT$
                        \State $Via \gets Via \ \cup \ \{v, vEntryPos, vExitPos\} $
                    \EndIf
                \EndIf 
            \EndFor 
        \EndIf 
    ...
    \If {$backward = false$}
        ...
        \If {$vOverlayVisited = false \textbf{ or } newTT < fInfo[vOverlay].tt$}
            ...
            \If {$visitedByBackward = true \textbf{ and } fInfo[vOverlay].tt + bInfo[vOverlay].tt  < shortest$} 
                ...
                \State $Via \gets Via \ \cup \ \{v, G.entryPos[arc,v], G.exitPos[arc,v]\} $
            \EndIf
        \EndIf
    \Else 
...
\begin{algorithmic}[1]
\setstretch{0.8}
\Procedure{BidirectionalOverlayGraphDijkstra}{$s,t,fInfo,bInfo,fQ, bQ, foQ, boQ, G, OG, shortest, Via$}
