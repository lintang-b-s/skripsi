...
\setstretch{0.9}
    \Procedure{findAlternativeRoutes}{$s,t: V, \tau_0: \mathbb{R}, k: \mathbb{Z}^+$}: $Set$
        ...
        \Procedure{calculatePlateau}{$\tau_{bd}, r,v, l_v, p_{s}, p_t, \tau_0$}: $\mathbb{R}$
             \State $plateau = 0$
             \State $u = v$
             \While{$u \neq \perp$} \Comment{backtrack}
                    \If{$u\in p_t  $} \Comment{$u \in spTree_{forward} \text{ dan } u \in spTree_{backward}$}
                        \State $cost_{uf}= (\tau_{bd}[u]-\tau_0)-(\tau_{bd}[p_{s}[u]]-\tau_0)$
             ...
             \State $u = v$
             \While{$u \neq \perp$} \Comment{backtrack}
                    \If{$u\in p_s  $} \Comment{$u \in spTree_{forward} \text{ dan } u \in spTree_{backward}$}
                        \State $cost_{ub}= r[u]-r[p_{t}[u]]$
