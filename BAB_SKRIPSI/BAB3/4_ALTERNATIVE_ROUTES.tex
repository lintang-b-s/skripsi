...
\begin{algorithm}
\caption{BDTDDijkstra menghitung kandidat \textit{via nodes} $X\subseteq V$} 
...
\setstretch{0.9}
    ...
    \While{$(Q_S \neq \emptyset \textbf{ or } Q_t\neq \emptyset \textbf{ and } \min \{Q_s.min(), Q_t.min()\})\leq (1+\epsilon) B$}
    ...
    \Procedure{findAlternativeRoutes}{$s,t: V, \tau_0: \mathbb{R}, k: \mathbb{Z}^+$}: $Set$
        ...
        \For {$candidate \in X$}
            ...
            \If{$\sigma_v\geq \gamma \cdot l_{Opt}$}
                \State \textbf{continue} 
            \EndIf
            \State $pl_v=$\Call{calculatePlateau}{$\tau_{bd},r,candidate.node,  l_v, p_{sbd}, p_{tbd}, tau_0$}
