    ...
    \Procedure{OnlineMapMatchingMHT}{$G(V,E),g_{1:k},\Delta t_{1:k}$}
        ...
        \For {$k=1 \textbf{ to }K$}
            ...
            \For{$\textbf{all }c\in C$} \Comment{filtering}
                \State $c.w\leftarrow \frac{c.w.ObservationProb(g_k,c)}{\sum_{c'\in C}c'.w.ObservationProb(g_k,c')}$
            \EndFor
